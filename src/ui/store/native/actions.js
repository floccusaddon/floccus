...
export const actionsDefinition = {
  ...
  async [actions.EXPORT_BOOKMARKS]({ commit, dispatch, state }, accountId) {
    ...
    Logger.download(
      'floccus-' + new Date().toISOString().slice(0, 10) + '.export.html',
      blob
    )
  },
  async [actions.TEST_WEBDAV_SERVER](
  ...
  async [actions.TEST_LINKWARDEN_SERVER](
    { commit, dispatch, state },
    { rootUrl, token }
  ) {
  ...
  async [actions.START_LOGIN_FLOW]({ commit, dispatch, state }, rootUrl) {
    ...
    do {
      ...
      (res.status === 404 || !res.data.appPassword) &&
      state.loginFlow.isRunning
    )
    commit(mutations.SET_LOGIN_FLOW_STATE, false)
    await Browser.close()
    if (res.status !== 200) {
