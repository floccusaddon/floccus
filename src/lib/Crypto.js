export default class Crypto {
  ...
  static bufferToHexstr (buffer) {
    return Array.from(new Uint8Array(buffer)).map(b => ('00' + b.toString(16)).slice(-2)).join('') // convert bytes to hex string
  }

  static hexstrToBuffer (hex) {
    for (var bytes = new Uint8Array(hex.length / 2), c = 0; c < hex.length; c += 2) {
  ...
  static async prepareKey (key) {
    const keyBuffer = await crypto.subtle.digest('SHA-256', new TextEncoder('utf-8').encode(key)) // hash the key
    let cryptoKey = await crypto.subtle.importKey('raw'
      , keyBuffer
      , {name: 'AES-CBC'}
      , false
      , ['decrypt', 'encrypt']
...
Crypto.iv = [58, 14, 9, 204, 174, 93, 77, 98, 12, 248, 11, 160, 143, 24, 119, 20]
