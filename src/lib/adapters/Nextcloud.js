...
export default class NextcloudAdapter {
        ...
        <form>
          <table>
            <tr>
              ...
              <td>
                <input
                  value={new InputInitializeHook(data.url)}
                  type="text"
                  className="url"
                  name="url"
                  ev-keyup={onchangeURL}
                  ev-blur={onchangeURL}
                />
              </td>
            </tr>
            <tr>
              ...
              <td>
                <input
                  value={new InputInitializeHook(data.username)}
                  type="text"
                  className="username"
                  name="username"
                  ev-keyup={onchangeUsername}
                  ev-blur={onchangeUsername}
                />
              </td>
            </tr>
            <tr>
              ...
              <td>
                <input
                  value={new InputInitializeHook(data.password)}
                  type="password"
                  className="password"
                  name="password"
                  ev-keydown={onchangePassword}
                  ev-blur={onchangePassword}
                />
              </td>
            </tr>
            <tr>
              ...
              <td>
                <input
                  value={new InputInitializeHook(data.serverRoot || '')}
                  type="text"
                  className="serverRoot"
                  name="serverRoot"
                  placeholder="Default: root folder  Example: /my/subfolder"
                  ev-keyup={onchangeServerRoot}
                  ev-blur={onchangeServerRoot}
                />
              </td>
            </tr>
            <tr>
              ...
              <td>
                ...
                <a
                  href="#"
                  className={
                    'btn forceSync ' + (data.syncing ? 'disabled' : '')
                  }
                  ev-click={() => !data.syncing && ctl.sync()}
                >
                  Sync now
                </a>
                ...
                <div className="options">
                  <formgroup>
                    ...
                    <a
                      ...
                      className={
                        'btn resetRoot ' + (data.syncing ? 'disabled' : '')
                      }
                      ev-click={() => {
                        !data.syncing &&
                          ctl.update({ ...data, localRoot: null })
                      }}
                    >
                      Reset
                    </a>
                    <a
                      ...
                      className={
                        'btn chooseRoot ' + (data.syncing ? 'disabled' : '')
                      }
                      ev-click={e => {
                        e.preventDefault()
                        ctl.pickFolder()
  ...
  async pullBookmarks() {
    ...
    let bookmarks = json.data.map(bm => {
      return new Bookmark(
        ...
        bm.url,
        bm.title,
        NextcloudAdapter.getPathFromServerMark(bm)
      )
  ...
  async updateBookmark(remoteId, newBm) {
    ...
    return new Bookmark(
      ...
      putJson.item.url,
      putJson.item.title,
      NextcloudAdapter.getPathFromServerMark(putJson.item)
    )
  }
  ...
  async removeBookmark(remoteId) {
    ...
    if (res.status !== 200) {
      throw new Error(
        'Removing a bookmark on the server failed. remoteId=' + remoteId
      )
    }
  }
